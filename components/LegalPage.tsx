      ...
      <main className="pt-24 min-h-screen">
        ...
        <div className="relative py-12 px-4 sm:px-6 lg:px-8 border-b border-white/5">
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-300 text-xs uppercase tracking-widest mb-4">
              Legal
            </div>
            <h1 className="text-3xl lg:text-4xl font-bold mb-3">{title}</h1>
            <p className="text-slate-400 mb-2">{subtitle}</p>
            <p className="text-xs text-slate-600">Última actualización: {lastUpdated}</p>
          </div>
        </div>

        {/* Content */}
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <div className="glass rounded-2xl border border-white/5 p-8 lg:p-12">
            <div className="prose-custom space-y-10">
              {sections.map((section, i) => (
                <section key={i}>
                  <h2 className="text-xl font-bold mb-4 flex items-center gap-3">
                    <span className="flex-shrink-0 w-7 h-7 rounded-lg bg-blue-500/15 border border-blue-500/20 flex items-center justify-center text-xs font-bold text-blue-400">
                      {i + 1}
                    </span>
                    {section.title}
                  </h2>
            ...
            <div className="mt-12 pt-8 border-t border-white/8">
              <p className="text-sm text-slate-500">
                ...
                <a href="mailto:legal@varsalsystems.com" className="text-blue-400 hover:underline">
